...
This article was written for desktop viewing. It won't throw errors if you try to read this on a mobile device, but the formatting will be wonky and the the interactive figures become almost impossible to navigate. So if you're able to, please read this on a desktop device.
...
from scipy.stats import gamma, lognorm, linregress
...
# 'Goed Nieuws' - Statistics about the podcast {#sec-statistics}
...
```{python}
data[is_ep].sort_values("date").iloc[0:1][print_cols].to_html()
```
...
For the loyal listeners, that's good new!
...
It does probably help that Maarten and Tom can talk about pretty much anything.
I wonder how much talking they actually do per episode, and is there a lot of variation in how much they talk? Let's find out in the next section.
...
As we can see in @fig-wpedist the content is quite varied in terms of how much is being said in the conversations. The extremely short end is of course the 5 seconds long 'Goed nieuws' episode, and some of the longest episodes are the specials.

```{python}
#| label: fig-wpetime
#| fig-cap: The distribution of Words-per-Episode over time, shown together with the duration of episodes over time. A clear trend is visible where episodes at the start of the podcast lasted more than ~50 minutes, while in the second half of the podcast episode duration dropped to less than ~40 minutes. At the same time we can see that the rate at which episodes were released increased rapidly, the density of datapoints increases by a lot! 
...
In @fig-wpetime we can see some interesting trends over time. 
This made me wonder, has the amount of content over time increased of decreased?
...
def plot_content(data):
    ...
    weeks = weeks[:,np.newaxis]
    a, _, _, _ = np.linalg.lstsq(weeks, WPW)
    plt.scatter(weeks, WPW)
    plt.plot(weeks, a*weeks, c='r', label="$y = ax$ \n"+f"a = {a[0]:.2f} [words-per-week]")
...
def plot_content_logistic(data):
    ...
    plt.plot(weeks, logifunc(weeks, *popt), 'r-',label='Fitted function')   
    plt.legend(loc="upper left")
    plt.xlabel("Weeks since start of podcast")
    plt.ylabel("Released content per week [words]")
    plt.title("The amount of content per week is steadily increasing")
...
```
# 'An image is worth 300 words' - Statistics on the words used {#sec-wordclouds}
...
# 'The talk of the town' - Topic Modeling on the Podcast! {#sec-topic}
...
# What is the episode that is most consistent to a single topic?
We may need to consider hierarchical clustering to find the correct answer to this question, or we could use our Umap to find the episode with the lowest RMS to the average embedding. Something like that could help.

# What is the episode that is the least consistent to a topic
...
For our analysis, it is better to ignore these specials, as they are not representative for typical podcast episodes. 
...
print(f"When we remove these episodes we have {num_episodes} episodes.")
...
print(f"For the loyal listeners, that's good new! As we have {data.episode.max()} episodes at the time of writing. It means that the average waiting time for a new episode is only {mean_time_between_episodes:.2f} days! That is more than {7/mean_time_between_episodes:.0f} episodes per week over the span of 2.5 years, wow!")
```
...
Before we answer that question, I would like to revisit our histogram, as @fig-wpe_time has given us some usefull information on how to filter our data to make a histogram
...
#| fig-cap: "There is a clear rising trend in the weekly data, as illustrated by the linear fit. The trend indicates that the amount of weekly content has been increasing with on average 144 words per week. One might notice that we could get a better fit by ommiting the first datapoint: first 3 episodes were all released on the same day, followed by a 3 month gap untill episode 4. Removing this first week and starting at episode 4 results in a slightly steeper slope of around 150 words-per-week."
...
## Maarten and Tom keep their talking rate very consistent throughout the episode
This talking rate hypothesis is a bit more difficult to answer, but it is certainly possible. Luckily for us `Whisper` also creates subtitle files for each processed episode (.srt, .vtt). We can process these to devide the text of each episode into minutes, so we can estimate the speech rate per minute. 
...
```{python}
# We do the same as above, but now for bigrams and above
path_top_n_words_sub_senteces = "./cache/top_n_words_sub_senteces.pickle"
if not os.path.exists(path_top_n_words_sub_senteces):
    word_freq_sub_senteces, bag_of_words_sub_senteces, vectorizer_sub_senteces = get_top_n_words(data, ngram_range=(5,5), minimum_rate=0.001, stopwords=[])
    package_sub_senteces = [word_freq_sub_senteces, bag_of_words_sub_senteces, vectorizer_sub_senteces]
    write_pickle(path_top_n_words_sub_senteces, package_sub_senteces)
else:
    package_sub_senteces = read_pickle(path_top_n_words_sub_senteces)
    word_freq_sub_senteces, bag_of_words_sub_senteces, vectorizer_sub_senteces = package_sub_senteces
word_freq_sub_senteces.iloc[:21].to_html(index=False)
```
...
This part of the analysis was largely inspired by the wonderful work of [Maarten Grootendorst](https://github.com/MaartenGr/BERTopic) and uses his `BERTopic` topic modeling pipeline. In short, the BERTopic algorithm works as follows:
...
2.  Dimensionality reduction algorithm (UMAP):\ 
High dimensional spaces such as the above are difficult to cluster due to the curse of dimensionality. We use a dimensionality reduction algorithm to reduce the number of dimensions to ~7. This algorithm tries to strike a balance between preserving local and global structure of the data. Such extreme dimensionality reduction means that we will lose information, but in exchange makes the data a lot more tractable for the clustering algorithm.
...
# Visualize documents
fig_visualize_documents = topic_model.visualize_documents(
    adjusted_docs,
...
#| column: page 
fig_hierarchical_documents = topic_model.visualize_hierarchical_documents(
    docs, 
    hierarchical_topics, 
    reduced_embeddings=reduced_embeddings,
...
fig_hierarchical_documents.show()
```

We see that @fig-topics_document_and_hierarchy combines @fig-topics_documents and @fig-topics_hierarchy into a single figure. Showing us the what the hierarchical clustering looks like in the 2 dimensional representation. By moving the slider we can further visualize the structure in the clustering algorithm. This reveals to us the general structure of topics as we identified ourselves in @fig-topics_documents.

## The podcast really is an 'actualiteiten' podcast
In the description of the podcast we may read: _'Maarten van Rossem en Tom Jessen geven hun kijk op het nieuws en de actualiteit.'_, which translates to: _'Maarten van Rossem and Tom Jessen share their point of view on the news and current affairs.'_.
With our current techniques, we can easily check if this is true. We already saw some proof of this in @fig-over_time_oekraine, but with our current topic model we can go a lot further. We can plot our top 12 topics over time, and check their relative frequencies. If this really is a current affairs podcast, we should be able to see some clear patterns over time. 
...
## What topic is each episode about? (Please don't let the stagair do this!)
```{python}
topic_names = {
    key: topic_model.custom_labels_[key + topic_model._outliers]
    for key, _ in topic_model.topic_labels_.items()
...
::: {.callout-note collapse="true"}
## Click here to see the top 3 topics identified for all episodes
```{python}
#| echo: false
topics_per_episode.to_html(index=False)
```
:::

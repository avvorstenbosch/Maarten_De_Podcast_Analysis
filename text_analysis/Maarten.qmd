---
title: "'Maarten van Rossem - De Podcast' - #315 Textanalysis of the podcast"
...
data = data[is_ep].reset_index(drop=True)
...
For the loyal listeners, that's good new! As we have 322 episodes at the time of writing.

```{python}
#| output: asis
#| echo: false
mean_time_between_episodes = days_running/num_episodes
print(f"It means that the average waiting time for a new episode is only {mean_time_between_episodes:.2f} days! That is more than {7/mean_time_between_episodes:.0f} episodes per week, wow!")
```

It does probably help that Maarten and Tom can talk about pretty much anything, given that they are both a history and a current affairs podcast. However, releasing many episodes doesn't mean a lot if there is only little content in each episode. Now, I feel quite satisfied with amount of content released by Maarten and Tom, but perhaps we can quantify it? I wonder how much talking they actually do per episode? Let's find out in the next section!
...
Content is hard to measure, so we just take the number of words spoken as a proxy for the content of each episode. This is easily visualised with a histogram.
```{python}
#| label: fig-wpe_dist
...
def get_length(text):
    """
    Calculates length of text by counting the number of tokens.
...
plot_lengths_histogram(data_full, title="All released episodes (specials included)", bins=20)
```

As we can see in @fig-wpe_dist the content is quite varied. Some episodes have almost 16.000 words, while others have only about 2000. But on the whole, most episodes have around 6.000 words. As it turns out, the episodes with the most words are the specials, which are also the longest episodes.

A histogram of course aggregates our data. Another fun way to visualize the content, is to plot all the individual datapoints over time. Will we be able to spot any trends over time?
...
model_params = plot_lengths_histogram_fit(data_second_half, title="Fitted Lognormal and Gamma Distribution", bins=20)
```
...
rmse_lineair_wpw = plot_content(data)
```
...
def plot_content_logistic(data):
    def weeks_since_start(date):
        time_diff = date - data.iloc[0].date
        weeks_diff = int(time_diff / np.timedelta64(1, 'W'))
        return weeks_diff
    ...
    plt.figure(figsize=figsize_inch)
    plt.scatter(weeks, WPW)
    plt.plot(weeks, logifunc(weeks, *popt), 'r-',label=f'Fitted Parameters {[round(p,1) for p in popt]}\nStarting-level={popt[3]:.1f}\nEnding-level ={popt[3]+popt[0]:.1f} \nRMSE={rmse_logistic_wpw:.2f}\n $R^2$ = {r_logistic_wpw:.2f}')
    plt.legend(loc="upper left")
    plt.xlabel("Weeks since start of podcast")
    plt.ylabel("Released content per week [words]")
    plt.title("The amount of content per week has almost tripled since the start of the podcast!")
    plt.ylim(0, 33000)
    plt.show()
    return mean_squared_error(WPW, logifunc(weeks, *popt), squared=False)


rmse_logistic_wpw = plot_content_logistic(data)
```
...
def plot_wpm_histogram(data):
    ...
    ax.plot(x, norm_fitted, label = "Normal Distribution:\n"+r"  - $\mu$ = "+f"{round(params[0],2)}\n  - $\sigma$ = {round(params[1],2)}")
...
def parse_srt(row):
    ...
    srt = pd.DataFrame({"lines":lines, "sentences":sentences, "timestamps":timestamps}) 
    ...
    srt["duration"] = pd.to_timedelta(srt["time_end"] - srt["time_start"]).dt.total_seconds()
...
def plot_srts_wpm(srts):
    ...
    plt.fill_between(date, ci_90[:,0], ci_90[:,1], color="#1f77b4", linewidth = 0, alpha=0.3, label=r"90% interval WPM")
    plt.fill_between(date, ci_60[:,0], ci_60[:,1], color="#1f77b4", linewidth = 0, alpha=0.5, label=r"60% interval WPM")
...
def plot_srts_wpm_weekly(srts):
    ...
    hpd = []
    weekdays = []
    number_to_week = {1:"Monday", 2:"Tuesday", 3:"Wednesday", 4:"Thursday", 5:"Friday", 6:"Saturday", 7:"Sunday"}
    for i in range(7):
    ...
    labels = df_weekday.groupby("weekday").agg({"weekday": lambda x: x.iloc[0], "episode":pd.Series.nunique})
    labels["label"] = labels.agg(lambda row: f"{number_to_week[row.weekday+1]}\n{row.episode} episodes", axis=1)

    fig, ax = plt.subplots(figsize=figsize_inch)
    plt.plot(weekdays, hpd, alpha=0.5, color="k", label="Highest Probability Density WPM")
    plt.fill_between(weekdays, ci_90[:,0], ci_90[:,1], color="#1f77b4", linewidth = 0, alpha=0.3, label=r"90% interval WPM")
    plt.fill_between(weekdays, ci_60[:,0], ci_60[:,1], color="#1f77b4", linewidth = 0, alpha=0.5, label=r"60% interval WPM")
    plt.xlabel("Weekday")
    plt.ylabel("Words-per-Minute")
    plt.title("Confidence Interval of WPM_lines")
    plt.legend()
...
srts_second_half = [srt for second, srt in zip((data["date"] > "2021-08-1"), srts) if second]
...
if not os.path.exists(path_wordcloud_full):
    ...
    wordcloud_full = WordCloud(width=figsize[0], height=figsize[1], include_numbers=True, random_state=2112, background_color="white", relative_scaling=1, max_words=300).generate(full_text)
...
if not os.path.exists(path_wordcloud_stopwords):
    wordcloud_stopwords = WordCloud(stopwords=dutch_stopwords, width=figsize[0], height=figsize[1], include_numbers=True, random_state=2112, background_color="white", relative_scaling=1, max_words=300).generate(full_text)
...
# 'Kunt u mij horen?' - What is 'Maarten van Rossem' the podcast actually about? {#sec-topic}
...
# Train and save our BERTopic model
...
if not os.path.exists(model_path):
    ...
    umap_model = UMAP(
        n_neighbors=20, #Higher -> More global structure, less local
    ...
    hdbscan_model = HDBSCAN(
        ...
        min_samples=7, #Lower -> less outlier classifications, may be detrimental to quality topics.
...
topics[1:10].to_html(index=False)
```
...
We end it here with @fig-topic_per_week_over_time